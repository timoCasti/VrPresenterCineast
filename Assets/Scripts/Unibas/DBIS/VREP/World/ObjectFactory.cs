...
namespace World
{
    ...
    public static class ObjectFactory
    {
        ...
        public static GameObject GetDisplayalPrefab()
        {
            ...
            {*/
            var prefab =
                Resources.Load("Prefabs/" + Settings.StandardDisplayalPrefabName, typeof(GameObject)) as GameObject;
            if (prefab == null)
                throw new Exception(string.Format("Could not load '{0}' as Resource",
        ...
        private static ComplexCuboidModel GenerateButtonModel(float size, float border, float height)
        {
            ...
            model.Add(new Vector3(border, border, -height),
                new CuboidModel(size - 2 * border, size - 2 * border, height));
            return model;
        }
