...
namespace Unibas.DBIS.VREP.World
{
    ...
    ///     The room is set up as its corresponding model is defined (RoomData). The virtual 3d appearance is driven by its
    ///     model.
    /// </summary>
    ...
    public class CuboidExhibitionRoom : MonoBehaviour
    {
        ...
        public void LoadAmbientAudio()
        {
            if (!string.IsNullOrEmpty(RoomData.GetURLEncodedAudioPath()))
            {
                ...
                if (_audioLoader == null) _audioLoader = gameObject.AddComponent<AudioLoader>();
