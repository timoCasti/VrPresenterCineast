...
namespace Unibas.DBIS.VREP
{
    public class VREPController : MonoBehaviour
    {
        ...
        {
            if (!Settings.VREMAddress.EndsWith("/")) Settings.VREMAddress += "/";

            if (!Settings.VREMAddress.StartsWith("http://")) Settings.VREMAddress = "http://" + Settings.VREMAddress;
        }
        ...
        private void Start()
        {
            ...
            if (go != null && Settings.StartInLobby) go.transform.position = new Vector3(0, -9.9f, 0);

            var lby = GameObject.Find("Lobby");
            if (lby != null && !Settings.StartInLobby) lby.SetActive(false);
        ...
        public void LoadAndCreateExhibition()
        {
            ...
            var exId = "";
            if (Settings.exhibitionIds != null && Settings.exhibitionIds.Length > 0 &&
                Settings.exhibitionIds[0] != null)
        ...
        private void Update()
        {
            if (!Settings.PlaygroundEnabled) return;

            if (Input.GetKey(KeyCode.F12)) _exhibitionManager.RestoreExhibits();
        }
