...
namespace Valve.VR.InteractionSystem
{
    ...
    public class ControllerButtonHints : MonoBehaviour
    {
        ...
        [SteamVR_DefaultAction("Haptic")] public SteamVR_Action_Vibration hapticFlash;
        ...
        private void CreateAndAddButtonInfo(SteamVR_Action_In action, SteamVR_Input_Sources inputSource)
        {
            ...
            {
                buttonDebug +=
                    string.Format("\nCan't find component transform for action: {0}. Component name: \"{1}\"",
                        action.GetShortName(), actionComponentName);
            }
        ...
        private void ComputeTextEndTransforms()
        {
            ...
            foreach (var hintInfo in actionHintInfos)
            {
                ...
                {
                    Debug.LogWarning("Invalid end position for: " + hintInfo.Value.textStartAnchor.name,
        ...
        private void ShowText(SteamVR_Action_In action, string text, bool highlightButton = true)
        {
            if (actionHintInfos.ContainsKey(action))
            {
                ...
                if (highlightButton) ShowButtonHint(action);
        ...
        public static void ShowButtonHint(Hand hand, params SteamVR_Action_In[] actions)
        {
            ...
            if (hints != null) hints.ShowButtonHint(actions);
        }
        ...
        {
            var hints = GetControllerButtonHints(hand);
            if (hints != null) hints.HideButtonHint(actions);
        }
        ...
        {
            var hints = GetControllerButtonHints(hand);
            if (hints != null) return hints.IsButtonHintActive(action);
        ...
        {
            var hints = GetControllerButtonHints(hand);
            if (hints != null) hints.ShowText(action, text, highlightButton);
        }
        ...
        {
            var hints = GetControllerButtonHints(hand);
            if (hints != null) hints.HideText(action);
        }
        ...
        {
            var hints = GetControllerButtonHints(hand);
            if (hints != null) return hints.GetActiveHintText(action);
