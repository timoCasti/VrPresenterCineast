...
namespace Valve.VR.InteractionSystem
{
    ...
    public class Balloon : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            {
                if (burstOnLifetimeEnd) SpawnParticles(lifetimeEndParticlePrefab, lifetimeEndSound);
        ...
        {
            // Don't do this twice
            if (bParticlesSpawned) return;

            bParticlesSpawned = true;

            if (particlePrefab != null)
            {
                var particleObject = Instantiate(particlePrefab, transform.position, transform.rotation);
        ...
        {
            // Slow-clamp velocity
            if (balloonRigidbody.velocity.sqrMagnitude > maxVelocity) balloonRigidbody.velocity *= 0.97f;
        }
