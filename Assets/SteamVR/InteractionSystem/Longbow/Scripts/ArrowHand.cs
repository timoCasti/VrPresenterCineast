...
namespace Valve.VR.InteractionSystem
{
    ...
    public class ArrowHand : MonoBehaviour
    {
        ...
        {
            var arrow = Instantiate(arrowPrefab, arrowNockTransform.position, arrowNockTransform.rotation);
            ...
            {
                var oldArrow = arrowList[0];
                arrowList.RemoveAt(0);
                if (oldArrow) Destroy(oldArrow);
            }
        ...
        private void HandAttachedUpdate(Hand hand)
        {
            ...
            if (!nocked)
            {
                ...
                {
                    var lerp = Util.RemapNumber(distanceToNockPosition, rotationLerpThreshold, lerpCompleteDistance, 0,
                        1);
                ...
                {
                    var posLerp = Util.RemapNumber(distanceToNockPosition, positionLerpThreshold, lerpCompleteDistance,
                        0, 1);
                ...
                {
                    if (arrowLerpComplete) arrowLerpComplete = false;
                }
                ...
                {
                    if (inNockRange) inNockRange = false;
                }

                var bestGrab = hand.GetBestGrabbingType(GrabTypes.Pinch, true);
                ...
                {
                    if (currentArrow == null) currentArrow = InstantiateArrow();
