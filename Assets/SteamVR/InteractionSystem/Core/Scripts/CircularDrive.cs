...
namespace Valve.VR.InteractionSystem
{
    ...
    public class CircularDrive : MonoBehaviour
    {
        ...
        private void Start()
        {
            if (childCollider == null) childCollider = GetComponentInChildren<Collider>();
            ...
            if (limited)
            {
                ...
                if (forceStart) outAngle = Mathf.Clamp(startAngle, minAngle, maxAngle);
            }
            else
            {
        ...
        private void OnHandHoverEnd(Hand hand)
        {
            ...
            if (driving && hand) StartCoroutine(HapticPulses(hand, 1.0f, 10));
        ...
        private void HandHoverUpdate(Hand hand)
        {
            ...
            var isGrabEnding = hand.IsGrabbingWithType(grabbedWithType) == false;
            ...
            if (driving && isGrabEnding == false && hand.hoveringInteractable == interactable)
            {
                ComputeAngle(hand);
                UpdateAll();
            }
        ...
        private Vector3 ComputeToTransformProjected(Transform xForm)
        {
            ...
            if (debugPath && dbgPathLimit > 0) DrawDebugPath(xForm, toTransformProjected);
        ...
        {
            if (rotateGameObject) transform.localRotation = start * Quaternion.AngleAxis(outAngle, localPlaneNormal);
        }
        ...
        {
            if (debugText) debugText.text = string.Format("Linear: {0}\nAngle:  {1}\n", linearMapping.value, outAngle);
        }
                ...
                if (absAngleDelta > 0.0f)
                {
                    if (frozen)
                    {
                        ...
                        if (frozenSqDist > frozenSqDistanceMinMaxThreshold.x)
                        {
                            ...
                            var magnitude = Util.RemapNumberClamped(frozenSqDist, frozenSqDistanceMinMaxThreshold.x,
                            ...
                            if (frozenSqDist >= frozenSqDistanceMinMaxThreshold.y) onFrozenDistanceThreshold.Invoke();
                        }
                    }
                    else
                    {
                        ...
                        if (dot < 0.0f) signedAngleDelta = -signedAngleDelta;
