...
{
    /// <summary>
    ///     This component simplifies the use of Pose actions. Adding it to a gameobject will auto set that transform's
    ///     position and rotation every update to match the pose.
    ...
    public class SteamVR_Behaviour_Pose : MonoBehaviour
    {
        ...
        protected virtual void CheckDeviceIndex()
        {
            if (poseAction.GetActive(inputSource))
                ...
                {
                    var currentDeviceIndex = (int) poseAction.GetDeviceIndex(inputSource);

                    if (deviceIndex != currentDeviceIndex)
                    {
                        deviceIndex = currentDeviceIndex;
                        gameObject.BroadcastMessage("SetInputSource", inputSource,
                            SendMessageOptions.DontRequireReceiver);
                        gameObject.BroadcastMessage("SetDeviceIndex", deviceIndex,
                            SendMessageOptions.DontRequireReceiver);
                    }
                }
        }
        ...
        {
            historyBuffer.Update(poseAction.GetLocalPosition(inputSource), poseAction.GetLocalRotation(inputSource),
                poseAction.GetVelocity(inputSource), poseAction.GetAngularVelocity(inputSource));
        }

        /// <summary>
        ///     Uses previously recorded values to find the peak speed of the pose and returns the corresponding velocity and
        ///     angular velocity
        /// </summary>
        public void GetEstimatedPeakVelocities(out Vector3 velocity, out Vector3 angularVelocity)
        {
            var top = historyBuffer.GetTopVelocity(10, 1);
