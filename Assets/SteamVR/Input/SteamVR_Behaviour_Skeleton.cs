...
namespace Valve.VR
{
    public class SteamVR_Behaviour_Skeleton : MonoBehaviour
    {
        ...
        /// <summary>
        ///     The range of motion you'd like the hand to move in. With controller is the best estimate of the fingers
        ///     wrapped around a controller. Without is from a flat hand to a fist.
        /// </summary>
        [Tooltip(
        ...
        /// <summary>
        ///     The root Transform of the skeleton. Needs to have a child (wrist) then wrist should have children in the order
        ...
        /// <summary>
        ///     Whether or not to update this transform's position and rotation inline with the skeleton transforms or if this
        ///     is handled in another script
        /// </summary>
        [Tooltip(
        ...
        {
            get { return skeletonAction.GetActive(inputSource); }
        }
        ...
        protected virtual void Awake()
        {
            ...
            proximals = new[] {thumbProximal, indexProximal, middleProximal, ringProximal, pinkyProximal};
            middles = new[] {thumbMiddle, indexMiddle, middleMiddle, ringMiddle, pinkyMiddle};
            distals = new[] {thumbDistal, indexDistal, middleDistal, ringDistal, pinkyDistal};
            tips = new[] {thumbTip, indexTip, middleTip, ringTip, pinkyTip};
            auxs = new[] {thumbAux, indexAux, middleAux, ringAux, pinkyAux};
        }
        ...
        ///     WithController being the best estimation of where fingers are wrapped around the controller (pressing buttons,
        ///     etc).
        ...
        protected void ResetTemporaryRangeOfMotionBlend(float blendOverSeconds)
        {
            if (temporaryRangeOfMotion != null)
            {
                ...
                var oldRangeOfMotion = temporaryRangeOfMotion.Value;
                ...
                if (skeletonBlend > 0)
                {
                    ...
                    for (var boneIndex = 0; boneIndex < bones.Length; boneIndex++)
                    {
                        ...
                        if (SteamVR_Utils.IsValid(newBoneRotations[boneIndex]) == false ||
                            SteamVR_Utils.IsValid(oldBoneRotations[boneIndex]) == false) continue;

                        var blendedRangeOfMotionPosition = Vector3.Lerp(oldBonePositions[boneIndex],
                            newBonePositions[boneIndex], lerp);
                        var blendedRangeOfMotionRotation = Quaternion.Lerp(oldBoneRotations[boneIndex],
        ...
        protected virtual void UpdateSkeletonTransforms()
        {
            ...
            var bonePositions = GetBonePositions(inputSource);
            var boneRotations = GetBoneRotations(inputSource);
        ...
        /// <summary>
        ///     Gets the position of the transform for a bone by the joint index. Joint indexes specified in:
        ...
        /// <summary>
        ///     Gets the rotation of the transform for a bone by the joint index. Joint indexes specified in:
        ///     SteamVR_Skeleton_JointIndexes
        /// </summary>
        /// <param name="joint">The joint index of the bone. Specified in SteamVR_Skeleton_JointIndexes</param>
        ...
        {
            var rawSkeleton = skeletonAction.GetBonePositions(inputSource);
            if (mirroring == MirrorType.LeftToRight || mirroring == MirrorType.RightToLeft)
                for (var boneIndex = 0; boneIndex < rawSkeleton.Length; boneIndex++)
        ...
        {
            var rawSkeleton = skeletonAction.GetBoneRotations(inputSource);
            if (mirroring == MirrorType.LeftToRight || mirroring == MirrorType.RightToLeft)
                for (var boneIndex = 0; boneIndex < rawSkeleton.Length; boneIndex++)
                {
                    ...
                    if (IsMetacarpal(boneIndex)) rawSkeleton[boneIndex] = rightFlipAngle * rawSkeleton[boneIndex];
                }
        ...
        protected virtual void UpdatePose()
        {
            ...
            if (origin == null)
            {
                skeletonAction.UpdateTransform(inputSource, transform);
            }
            else
            {
                transform.position = origin.TransformPoint(skeletonAction.GetLocalPosition(inputSource));
                transform.eulerAngles =
                    origin.TransformDirection(skeletonAction.GetLocalRotation(inputSource).eulerAngles);
            }
        }
