...
namespace Valve.VR
{
    ...
    public class SteamVR_Input_ActionFile
    {
        ...
        public void SaveHelperLists()
        {
            ...
            {
                var localizationItem = new Dictionary<string, string>();
                localizationItem.Add(SteamVR_Input_ActionFile_LocalizationItem.languageTagKeyName, item.language);

                foreach (var itemItem in item.items) localizationItem.Add(itemItem.Key, itemItem.Value);
        ...
        public static string GetShortName(string name)
        {
            ...
            if (lastSlash != -1)
            {
                if (lastSlash == fullName.Length - 1)
                {
                    ...
                    if (lastSlash == -1) return GetCodeFriendlyName(fullName);
                }
        ...
        public string[] GetFilesToCopy(bool throwErrors = false)
        {
            ...
            var actionFileInfo = new FileInfo(SteamVR_Input.actionsFilePath);
            var path = actionFileInfo.Directory.FullName;

            files.Add(SteamVR_Input.actionsFilePath);

            foreach (var binding in default_bindings)
            {
                var bindingPath = Path.Combine(path, binding.binding_url);
                ...
                else
                {
                    if (throwErrors)
                        Debug.LogError(
                            "[SteamVR] Could not bind binding file specified by the actions.json manifest: " +
                            bindingPath);
                }
            }
    ...
    public class SteamVR_Input_ActionFile_Action
    {
        ...
        public SteamVR_Input_ActionFile_Action_Requirements requirementEnum
        {
            get
            {
                for (var index = 0; index < requirementValues.Length; index++)
        ...
        public string path
        {
            get
            {
                ...
                if (lastIndex != -1 && lastIndex + 1 < name.Length) return name.Substring(0, lastIndex + 1);
    ...
    public static class SteamVR_Input_ActionFile_ActionTypes
    {
        ...
        public static string[] listAll = {boolean, vector1, vector2, vector3, vibration, pose, skeleton};
        public static string[] listIn = {boolean, vector1, vector2, vector3, pose, skeleton};
        public static string[] listOut = {vibration};
        public static string[] listSkeletons = {skeletonLeftPath, skeletonRightPath};
    }
    ...
    public static class SteamVR_Input_ActionFile_ActionSet_Usages
    {
        ...
        public static string[] listValues = {leftright, single, hidden};
        public static string[] listDescriptions = {leftrightDescription, singleDescription, hiddenDescription};
    }
}