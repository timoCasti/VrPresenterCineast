...
{
    /// <summary>
    ///     Skeleton Actions are our best approximation of where your hands are while holding vr controllers and pressing
    ///     buttons. We give you 31 bones to help you animate hand models.
    ...
    public class SteamVR_Action_Skeleton : SteamVR_Action_Pose
    {
        ...
        [NonSerialized] protected Dictionary<SteamVR_Input_Sources, EVRSkeletalMotionRange> rangeOfMotion =
            new Dictionary<SteamVR_Input_Sources, EVRSkeletalMotionRange>(new SteamVR_Input_Sources_Comparer());

        [NonSerialized] protected Dictionary<SteamVR_Input_Sources, EVRSkeletalTransformSpace> skeletalTransformSpace =
            new Dictionary<SteamVR_Input_Sources, EVRSkeletalTransformSpace>(new SteamVR_Input_Sources_Comparer());

        [NonSerialized] protected uint skeletonActionData_size;

        [NonSerialized] protected VRBoneTransform_t[] tempBoneTransforms = new VRBoneTransform_t[numBones];

        [NonSerialized] protected InputSkeletalActionData_t tempSkeletonActionData;
        ...
        public override void UpdateValue(SteamVR_Input_Sources inputSource, bool skipStateAndEventUpdates)
        {
            ...
            var poseChanged = changed[inputSource];
            ...
            var err = OpenVR.Input.GetSkeletalActionData(handle, ref tempSkeletonActionData, skeletonActionData_size,
                SteamVR_Input_Source.GetHandle(inputSource));
            if (err != EVRInputError.None)
            {
                Debug.LogError("GetSkeletalActionData error (" + fullPath + "): " + err + " handle: " + handle);
            ...
            {
                err = OpenVR.Input.GetSkeletalBoneData(handle, skeletalTransformSpace[inputSource],
                    rangeOfMotion[inputSource], tempBoneTransforms, SteamVR_Input_Source.GetHandle(inputSource));
                if (err != EVRInputError.None)
                    Debug.LogError("GetSkeletalBoneData error (" + fullPath + "): " + err + " handle: " + handle);
            ...
            if (skipStateAndEventUpdates == false)
            {
                for (var boneIndex = 0; boneIndex < tempBoneTransforms.Length; boneIndex++)
                {
                    if (Vector3.Distance(lastBonePositions[inputSourceInt][boneIndex],
                            bonePositions[inputSourceInt][boneIndex]) > changeTolerance)
                    {
                        changed[inputSource] |= true;
                        break;
                    }

                    if (Mathf.Abs(Quaternion.Angle(lastBoneRotations[inputSourceInt][boneIndex],
                            boneRotations[inputSourceInt][boneIndex])) > changeTolerance)
                    {
                        changed[inputSource] |= true;
                        break;
                    }
        ...
        /// <summary>
        ///     Set the range of the motion of the bones in this skeleton. Options are "With Controller" as if your hand is holding
        ///     your VR controller.
        ...
        /// <summary>
        ///     Sets the space that you'll get bone data back in. Options are relative to the Model, relative to the Parent bone,
        ///     and Additive.
        /// </summary>
        /// <param name="inputSource">The device you would like to get data from. Any if the action is not device specific.</param>
        /// <param name="space">
        ///     the space that you'll get bone data back in. Options are relative to the Model, relative to the
        ///     Parent bone, and Additive.
        /// </param>
    ...
    /// <summary>
    ///     The change in range of the motion of the bones in the skeleton. Options are "With Controller" as if your hand is
    ///     holding your VR controller.
    ///     Or "Without Controller" as if your hand is empty.
    /// </summary>
