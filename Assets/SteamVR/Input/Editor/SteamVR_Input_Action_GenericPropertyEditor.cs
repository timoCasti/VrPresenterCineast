...
namespace Valve.VR
{
    public class SteamVR_Input_Action_GenericPropertyEditor<T> : PropertyDrawer where T : SteamVR_Action
    {
        ...
        protected void Awake()
        {
            ...
            {
                var enumList = actions.Select(action => action.fullPath).ToList();
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            {
                var action = (T) property.objectReferenceValue;

                if (string.IsNullOrEmpty(action.fullPath) == false)
                    for (var actionsIndex = 0; actionsIndex < actions.Length; actionsIndex++)
