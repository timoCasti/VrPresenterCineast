...
{
    /// <summary>
    ///     Vector3 actions are relatively uncommon. If you're looking for an action to get a 3d position from you probably
    ///     want a Pose action.
    /// </summary>
    public class SteamVR_Action_Vector3 : SteamVR_Action_In
    {
        [NonSerialized] protected Dictionary<SteamVR_Input_Sources, InputAnalogActionData_t> actionData =
            new Dictionary<SteamVR_Input_Sources, InputAnalogActionData_t>(new SteamVR_Input_Sources_Comparer());

        [NonSerialized] protected uint actionData_size;

        [NonSerialized] protected Dictionary<SteamVR_Input_Sources, InputAnalogActionData_t> lastActionData =
            new Dictionary<SteamVR_Input_Sources, InputAnalogActionData_t>(new SteamVR_Input_Sources_Comparer());

        [NonSerialized] protected InputAnalogActionData_t tempActionData;
        ...
        public override void UpdateValue(SteamVR_Input_Sources inputSource)
        {
            ...
            var err = OpenVR.Input.GetAnalogActionData(handle, ref tempActionData, actionData_size,
                SteamVR_Input_Source.GetHandle(inputSource));
            if (err != EVRInputError.None)
                Debug.LogError("Vector3 GetAnalogActionData error (" + fullPath + "): " + err + " handle: " + handle);
            ...
            if (onUpdate[inputSource] != null) onUpdate[inputSource].Invoke(this);
        ...
        public Vector3 GetLastAxisDelta(SteamVR_Input_Sources inputSource)
        {
            return new Vector3(lastActionData[inputSource].deltaX, lastActionData[inputSource].deltaY,
                lastActionData[inputSource].deltaZ);
        }
    }
}